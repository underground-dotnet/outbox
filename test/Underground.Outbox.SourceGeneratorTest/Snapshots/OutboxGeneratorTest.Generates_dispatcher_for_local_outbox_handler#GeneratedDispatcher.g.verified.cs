...
public class GeneratedDispatcher<TMessage> : IMessageDispatcher<TMessage> where TMessage : class, IMessage
{
    public async Task ExecuteAsync(IServiceScope scope, TMessage message, CancellationToken cancellationToken)
    {
        ...
        {
            if (string.Equals(message.Type, typeof(Sample.TestMessage).FullName, StringComparison.Ordinal))
            {
                var fullEvent = JsonSerializer.Deserialize<Sample.TestMessage>(message.Data) ?? throw new ParsingException($"Cannot parse event body {message.Data} of message: {message.Id}");
                var handler = serviceProvider.GetRequiredService<IOutboxMessageHandler<Sample.TestMessage>>();
                try
                {
