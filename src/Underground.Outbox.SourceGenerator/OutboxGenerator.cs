...
public sealed class OutboxGenerator : IIncrementalGenerator
{
    ...
    private const string DiscardOnAttributeFullName = "Underground.Outbox.Attributes.DiscardOnAttribute";
    ...
    {
        var discardOnExceptionTypes = new EquatableList<string>();

        var interfaceHandleMethod = handlerInterface.GetMembers("HandleAsync")
            .OfType<IMethodSymbol>()
            .SingleOrDefault();

        if (interfaceHandleMethod is null)
        {
            return discardOnExceptionTypes;
        }

        if (typeSymbol.FindImplementationForInterfaceMember(interfaceHandleMethod) is not IMethodSymbol handleMethod)
        {
            return discardOnExceptionTypes;
        }
        ...
        {
            if (attribute.AttributeClass?.ToDisplayString() != DiscardOnAttributeFullName)
            {
                continue;
            }
            ...
            {
                AppendDiscardOnExceptionTypes(argument, discardOnExceptionTypes);
            }
        }
    ...
    private static void AppendDiscardOnExceptionTypes(TypedConstant argument, EquatableList<string> discardOnExceptionTypes)
    {
        if (argument.Kind == TypedConstantKind.Array)
        {
            foreach (var value in argument.Values)
            {
                AppendDiscardOnExceptionTypes(value, discardOnExceptionTypes);
            }

            return;
        }

        if (argument.Value is ITypeSymbol exceptionType)
        {
            discardOnExceptionTypes.Add(exceptionType.ToDisplayString());
        }
    }

    private static void GenerateDispatcher(EquatableList<HandlerClassInfo> handlers, SourceProductionContext context)
    {
    ...
    {
        var handlersWithDiscardMappings = handlers
            .Where(handler => handler.DiscardOnExceptionTypeFullNames.Count > 0)
            .ToList();
        ...
        sb.AppendLine("    {");
        foreach (var handler in handlersWithDiscardMappings)
        {
